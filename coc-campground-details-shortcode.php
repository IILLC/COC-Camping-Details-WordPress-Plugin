...
*/
	...
	if(!in_array($_SERVER['REMOTE_ADDR'], $whitelist)){
		// remote
		require('/home/campou5/public_html/wp-content/plugins/coc_campground-details/coc-fish-nearby-water-stock-info.php');
	} else { //local
		require('E:/Programs/xampp/htdocs/campoutcolorado.com/wp-content/plugins/coc_campground-details/coc-fish-nearby-water-stock-info.php');
//OLD		require('/coc-fish-nearby-water-stock-info.php');
	}
//require('/coc-fish-nearby-water-stock-info.php');
...
//example code given at http://developer.active.com/docs/read/Campground_Details_API
//http://api.amp.active.com/camping/campground/details?contractCode=CO&parkId=50032&api_key=r65cqxbb8phn3c93fvsm7d8
...
//		//debugging
//		print_r('DATE STAMPS');
//		print_r("         strtotime('-1 day')= ");
//		print_r(strtotime('-1 day'));
//		print_r('     $cocResults->camp_updated=   ');		
//		print_r($cocResults->camp_updated);
//		print_r('     strtotime($cocResults->camp_updated)=   ');		
////		$strtime = $cocResults->camp_updated;
//		print_r(strtotime($cocResults->camp_updated));
		
		if (strtotime('-1 day') > strtotime($cocResults->camp_updated)) {
			//if longer than 1 day since update, update database fields
			//print_r("Been longer than 1 day since update. Fire up the API!!!");
			//my api
			$activeAPI = 'sr65cqxbb8phn3c93fvsm7d8';
			//Example API URL CALL   http://api.amp.active.com/camping/campground/details?contractCode=CO&parkId=50032&api_key=sr65cqxbb8phn3c93fvsm7d8
			$activeURL = 'http://api.amp.active.com/camping/campground/details?contractCode='.$cCode.'&parkId='.$pID.'&api_key='.$activeAPI; //'sr65cqxbb8phn3c93fvsm7d8'
...
//  echo 'Took = '. $info['total_time'].' seconds to send a request to '. $info['url'] .' which returned with code '. $info['http_code'].' . The content type is '.$info['content_type'].' . The length of the call is '.$info['download_content_length'].' The redirect url is '.$info['redirect_url'].'</br>';
//} 
//
...
//			print_r($activeURL);	
//			echo '<br/>';		
//			print_r("Returned Result= ");
//			echo '<br/>';
//			print_r($result);			
//			echo '<br/>';
//			print_r("Making API CALL XML RETURN TO FOLLOW= ");
//			echo '<br/>';
//			print_r($xml);
//			echo '<br/>';
...
//			
//			echo '<br/>';
//			print_r("Status code of last cURL call= ");
//			echo '<br/>';
//			print_r(empty($result));
//			echo '<br/>';
//			print_r("String Length of full resrevation URL= ");
//			echo '<br/>';
//			print_r(strlen($xml['fullReservationUrl']));
//			echo '<br/>';
//			
//			print_r(" =End API Call");
///////////////////////////////////////////////
		
			//don't update if no connection/loss of data
		
			if  (!empty($xml)) {
				//have data, check update specific fields
				$campTable = 'coc_campground_details';
				$nowDate = date("Y-m-d H:i:s");
				if (!empty($xml['fullReservationUrl']) && strlen($xml['fullReservationUrl']) > 10){  
					//update url
//					print_r("ADDING FULL RESURL==================================================   ");
//					print_r($xml['fullReservationUrl']);
//					print_r(" ====================================END FULL RES URL");					

// DO NOT UPDATE URL UNTIL THEY FIX THEIR FUCKING API
//					$wpdb->update(
//								$campTable, 
//								array (
//									'reserve_url' => $xml['fullReservationUrl'],
...
//								);
				...
				//Update Amenities //time to parse out the amenities
...
//				}
		...
		// https://www.google.com/maps/d/viewer?hl=en&authuser=0&mid=1NOd9T5rPujjSZ1h7bQ0CX_x-jEE&z=14&ll=39.319694%2C-106.643564
		...
		if($res == 1) {
			//	Chapman Test Link	http://www.tantracking.com/click.track?CID=177360&AFID=269137&AffiliateReferenceID=-269137&nonencodedurl=http%3A%2F%2Fwww.reserveamerica.com%2FcampsiteSearch.do%3FcontractCode%3DNRSO%26parkId%3D70422%26cmp=39-19--269137 
...
<?php
//adding a line if there is any water nearby
